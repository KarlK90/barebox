...
struct resource *bootm_load_uimage_initrd(struct image_data *data, unsigned long load_address);
...
static inline int bootm_load_uimage_os(struct image_data *data, unsigned long load_address)
{
    return -ENOSYS;
}
...
static inline int bootm_open_oftree_uimage(struct image_data *data, size_t *size,
					   struct fdt_header **fdt)
{
    return -ENOSYS;
}
